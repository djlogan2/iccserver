...
describe.only("AnalysisEngines", function() {
  ...
  it.skip("should set an existing engine record busy, assign our ID, and return the engine ID when one is available", function() {
    sandbox.replace(AnalysisEngines, "intervalWait", 1);
    const fakeec2 = new FakeEc2(
      { startup_wait: 3, ssh_wait: 3 },
    ...
    sandbox.replace(AnalysisEngines, "getSSHClient", sandbox.fake.returnss(fakeec2.fakeSSH()));
    let instance_up = 0;
  ...
  describe("Each individual function", function() {
    beforeEach(() => {
      sandbox.replace(AnalysisEngines, "handleStatus", () => {
        console.log("yay");
        return null;
      });
    });
