...
class DynamicRatings {
  constructor() {
    this.collection = new Mongo.Collection("ratings");
    this.collection.attachSchema(RatingSchema);
  }
  addRatingType(
    message_identifier,
    wild_number,
    ...
    if (this.collection.find({ rating_type: rating_type }).count() !== 0)
  ...
  deleteRatingType(message_identifier, rating_type) {
    const self = Meteor.user();
    check(self, Object);
    check(message_identifier, String);
    check(rating_type, String);
  ...
  getUserRatingsObject() {
    const ratingRecords = this.collection
      .find({}, { fields: { rating_type: 1, default_rating: 1 } })
      .fetch();
    const ratingobject = {};
  ...
  meetsRatingTypeRules(
    ...
    const rating_object = this.collection.findOne({ rating_type: rating_type });
...
if (!global._dnrObject) {
  global._dnrObject = new DynamicRatings();
}

module.exports.DynamicRatings = global._dnrObject;

Meteor.publish("DynamicRatings", function () {
  return global._dnrObject.collection.find();
});

Meteor.startup(function () {
  if (!Meteor.isTest && !Meteor.isAppTest && global._dnrObject.collection.find().count() === 0) {
    global._dnrObject.collection.insert({
      wild_number: 0,
      rating_type: "bullet",
      white_initial: [0, 2],
