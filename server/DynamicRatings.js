...
function validateAndFillRatingObject(message_identifier, obj) {
  ...
  if (obj.initial && (!Array.isArray(obj.initial) || obj.initial.length !== 2)) throw new ICCMeteorError(message_identifier, "Unable to add rating", "Initial time must be an array of 'low, high'");
  if (obj.increment && (!Array.isArray(obj.increment) || obj.increment.length !== 2)) throw new ICCMeteorError(message_identifier, "Unable to add rating", "Increment/delay must be an array of 'low, high'");
  if (obj.etime && (!Array.isArray(obj.etime) || obj.etime.length !== 2)) throw new ICCMeteorError(message_identifier, "Unable to add rating", "etime must be an array of 'low, high'");

  if (obj.increment && !obj.increment_type) throw new ICCMeteorError(message_identifier, "Unable to add rating", "increment/delay type must be specified if increment/delay is specified");

  if (obj.increment_type && !Array.isArray(obj.increment_type)) throw new ICCMeteorError(message_identifier, "Unable to add rating", "increment/delay type must be an array of allowed increment/delay types");

  if (obj.initial && obj.initial[0] > obj.initial[1]) throw new ICCMeteorError(message_identifier, "Unable to add rating", "Initial time must be an array of 'low, high'");
  if (obj.increment && obj.increment[0] > obj.increment[1]) throw new ICCMeteorError(message_identifier, "Unable to add rating", "Increment/delay must be an array of 'low, high'");
  if (obj.etime && obj.etime[0] > obj.etime[1]) throw new ICCMeteorError(message_identifier, "Unable to add rating", "etime must be an array of 'low, high'");

  const unique = [];
  const dups = [];
  if (obj.increment_type)
    obj.increment_type.forEach(type => {
      if (allowed_increment_types.indexOf(type) === -1) throw new ICCMeteorError(message_identifier, "Unable to add rating", type + " is not an allowed increment/delay type");
      if (unique.indexOf(type) === -1) unique.push(type);
      else dups.push(type);
    });

  if (dups.length !== 0) throw new ICCMeteorError(message_identifier, "Unable to add rating", "increment/delay has duplicate entries.");

  if ((!obj.increment || obj.increment[1]) && (obj.increment_type && obj.increment_type.length === 1 && obj.increment_type[0] === "none")) throw new ICCMeteorError(message_identifier, "Unable to add rating", "increment/delay type must have at least one option when increment is available");
  ...
  if (obj.initial || obj.increment) {
    if (obj.etime) {
      ...
      if (calculated_low_etime !== obj.etime[0]) throw new ICCMeteorError(message_identifier, "Unable to add rating", "etime does not match", "etime of " + calculated_low_etime + " is not equal to the specified low etime of " + obj.etime[0]);
      if (calculated_high_etime !== obj.etime[1]) throw new ICCMeteorError(message_identifier, "Unable to add rating", "etime does not match", "etime of " + calculated_high_etime + " is is not equal to the specified high etime of " + obj.etime[1]);
    } else {
      // time = initial + inc
      obj.etime = [Math.trunc(obj.initial[0] + (2 * obj.increment[0]) / 3), Math.round(obj.initial[1] + (2 * obj.increment[1]) / 3)];
    }
  } else if (obj.etime) {
    ...
    const increment_disallowed = !!obj.increment_type && obj.increment_type.length === 1 && obj.increment_type[0] === "none";
    ...
    throw new ICCMeteorError(message_identifier, "Unable to add rating", "Initial, increment, and etime cannot all be null");
  }
  if (obj.increment[0]) {
    if (obj.increment_type.indexOf("none") !== -1) throw new ICCMeteorError(message_identifier, "Unable to add rating", "'none' cannot be specified when inc/delay is required");
  } else {
    //!obj.increment[0]
    if (obj.increment_type.indexOf("none") === -1) throw new ICCMeteorError(message_identifier, "Unable to add rating", "'none' is required when inc/delay can be zero");
  }
  if (obj.increment[1]) {
    if (obj.increment_type.length === 1 && obj.increment_type[0] === "none") throw new ICCMeteorError(message_identifier, "Unable to add rating", "increment or delay types are required when inc/delay can be non-zero");
  } else {
    // !obj.increment[1]
    if (obj.increment_type.length !== 1 || obj.increment_type[0] !== "none") throw new ICCMeteorError(message_identifier, "Unable to add rating", "'none' must be the only rating type when inc/delay cannot be set");
  }
  return obj;
}

DynamicRatings.addRatingType = function(message_identifier, wild_number, rating_type, rated, unrated, white_initial, white_increment_or_delay, white_increment_or_delay_type, white_etime, black_initial, black_increment_or_delay, black_increment_or_delay_type, black_etime, specify_color, can_seek, can_match, starting_rating) {
  ...
  if (!Users.isAuthorized(self, "add_dynamic_rating")) throw new ICCMeteorError(message_identifier, "Unable to add rating", "User not authorized");

  if (DynamicRatingsCollection.find({ rating_type: rating_type }).count() !== 0) throw new ICCMeteorError(message_identifier, "Unable to add rating", "Rating type already exists");

  if (!starting_rating) starting_rating = 1600;
  wild_number = wild_number || [0];
  if (wild_number.length !== 1 || wild_number[0] !== 0) throw new Match.Error("wild_number can only be zero");
  if (!rated && !unrated) throw new Match.Error("Unusable rating type. Both rated and unrated are false");
...
DynamicRatings.deleteRatingType = function(message_identifier, rating_type) {
  ...
  if (!Users.isAuthorized(self, "delete_dynamic_rating")) throw new ICCMeteorError(message_identifier, "Unable to add rating", "User not authorized");
...
DynamicRatings.getUserRatingsObject = function() {
  const ratingRecords = DynamicRatingsCollection.find({}, { fields: { rating_type: 1, default_rating: 1 } }).fetch();
  const ratingobject = {};
...
DynamicRatings.meetsRatingTypeRules = function(message_identifier, color, rating_type, time, inc_or_delay, inc_or_delay_type, rated, check_type, color_specified) {
  ...
  if (color !== "white" && color !== "black") throw new ICCMeteorError(message_identifier, "Invalid call", "Invalid color");
  if (!rating_object) throw new ICCMeteorError(message_identifier, "Invalid call", "Invalid rating type");
  ...
  if (color_specified && !rating_object.specify_color) throw new ICCMeteorError(message_identifier, "Invalid call", "Cannot specify color");
  ...
  if (inc && inctype === "none") throw new ICCMeteorError(message_identifier, "Invalid call", "Cannot specify inc/delay with a type of 'none'");
