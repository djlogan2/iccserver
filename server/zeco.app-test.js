...
describe.only("ecocodes", function(){
  ...
  describe.skip("recursive_load_eco", function() {
  ...
  describe.only("loadFen", function() {
    ...
    it.only("should not load an eco code for a fresh game with no moves", function() {
      this.timeout(500000000);
      ...
      const game_id = Game.startLocalGame("mi1", player, 0, "standard", true, 15, 15, "inc", 15, 15, "inc");
      // check node
    ...
    it.only("should not load an eco code for a fen without an eco entry", function() {
      // Create game with new board
      const player = TestHelpers.createUser();
      self.loggedonuser = player
      const game_id = Game.startLocalGame("mi1", player, 0, "standard", true, 15, 15, "inc", 15, 15, "inc");
      ...
      const game = Game.getAndCheck(self.loggedonuser,"mi1", game_id);
      // load fen
      const fen_string = game.fen;
      Game.loadFen("mi1", game_id, fen_string);
      const game2 = Game.GameCollection.findOne({status: "examining", _id: game_id});
      // check node again
      chai.assert.equal(game2.variations.movelist[0].eco.code, "NO_ECO");
      chai.assert.equal(game2.variations.movelist[0].eco.name, "NO_ECO");
    });
    it.only("should load an eco code for a fen with an eco entry", function() {
      // Create game with new board
      if (!Game.ecoCollection) Game.ecoCollection = new Mongo.Collection("ecocodes");
      let name = "King's Indian Attack";
      let code = "A07";
