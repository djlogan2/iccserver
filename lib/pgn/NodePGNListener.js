...
NodePGNListener.prototype.exitPgn_game = function(ctx) {
  ...
  if (this.gamelist.length === 1000) {
    ...
    //for (let x = 0 ; x < this.gamelist.length ; x++) delete this.gamelist[x];
    //delete this.gamelist;
    this.gamelist = [];
  }
};

NodePGNListener.prototype.exitTag_name = function(ctx) {
  this.tag = removeOuterQuotes(ctx.getText());
};

NodePGNListener.prototype.exitTag_value = function(ctx) {
  const value = removeOuterQuotes(ctx.getText());
  switch (this.tag) {
    case "White":
  ...
  this.currentgame.tags[this.tag] = value;
};
