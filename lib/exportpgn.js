  ...
  if (hasUI) {
    string = [
      <span
        ...
        style={{
          cursor: "pointer",
          color: cmi === active_cmi ? "red" : "black",
          whiteSpace: "nowrap",
        }}
...
function allVariations(movelist, cmi, move_number, white_to_move, hasUI, gameId, active_cmi) {
  ...
  variations.forEach((v) => {
    if (!hasUI) {
      string += "(" + thisMove(movelist[v], move_number, true, white_to_move, v, hasUI, gameId, active_cmi);
      ...
      const string_var1 = " (";
      const string_var2 = ")";
      string = [<span style={{whiteSpace: "pre-wrap"}}> </span>, ...string, <span style={{whiteSpace: "pre-wrap"}}>{string_var1}</span>, ...this_move];
      const nextmove = nextMove(
        movelist,
        v,
        next_move_number,
