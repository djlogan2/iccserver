...
function thisMove(
  ...
  if (hasUI) {
    string = [
      <span
        key={cmi}
        style={{ cursor: "pointer", color: cmi === active_cmi ? "red" : "black" }}
...
function allVariations(movelist, cmi, move_number, white_to_move, hasUI, gameId, active_cmi) {
  ...
  variations.forEach((v) => {
    if (!hasUI) {
      string +=
        "(" + thisMove(movelist[v], move_number, true, white_to_move, v, hasUI, gameId, active_cmi);
      ...
      const string_var1 = "(";
      const string_var2 = ")";
      string = [...string, <span>{string_var1}</span>, ...this_move];
      const nextmove = nextMove(
        movelist,
        v,
        next_move_number,
      ...
      if (nextmove) string = [...string, <span> </span>, ...nextmove];
...
function nextMove(movelist, cmi, move_number, white_to_move, hasUI = false, gameId, active_cmi) {
  if (!movelist[cmi].variations && !hasUI) return "";
  if (!movelist[cmi].variations && hasUI) return [];
  ...
  if (!!nextmove) {
    ...
    if (!!variations && variations.length && white_to_move && hasUI)
      string = [...string, <span>{next_move_number}. ...</span>];

    if (!hasUI) string += " " + nextmove;
    if (hasUI) string = [...string, <span> </span>, ...nextmove];
  }

  return string;
}

export function buildPgnFromMovelist(movelist, hasUI = false, gameId, active_cmi) {
  let long_string = nextMove(movelist, 0, 1, true, hasUI, gameId, active_cmi);
