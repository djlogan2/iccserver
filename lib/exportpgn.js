...
function allVariations(movelist, cmi, move_number, white_to_move, hasUI, gameId, active_cmi, classes) {
  ...
  variations.forEach((v) => {
    if (!hasUI) {
      string += "(" + thisMove(movelist[v], move_number, true, white_to_move, v, hasUI, gameId, active_cmi, classes);
      ...
      const string_var1 = " (";
      const string_var2 = ")";
      string = [<span style={classes?.break}> </span>, ...string, <span style={classes?.break}>{string_var1}</span>, ...this_move];
      const nextmove = nextMove(
        movelist,
        v,
        next_move_number,
