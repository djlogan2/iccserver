...
class Logger {
  ...
  static _log(clientserver, identifier, level, message, data, userid) {
    ...
    let now = new Date();
    let msg = "[" + clientserver + "]" + now.toString() + " [" + valid_levels[level] + "] " + identifier;
    ...
    // eslint-disable-next-line no-console
    if (Meteor.absoluteUrl() === "http://localhost:3000/" && !Meteor.isTest && !Meteor.isAppTest) console.log(msg);
  }
...
class SetupLogger {
  ...
  static addLogger(identifier, level) {
    if (valid_levels.indexOf(level.toUpperCase()) === -1) throw new Error("Level must be: " + valid_levels);
  ...
  static addLoggers(map) {
    for (let k in map) {
      if (map.hasOwnProperty(k) && valid_levels.indexOf(map[k]) === -1) throw new Error("Level must be " + valid_levels + " for " + k);
    }
    for (let k in map) {
