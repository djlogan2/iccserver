...
import ServerSlackNotifier from "./ServerSlackNotifier";

const serverSlackNotifier = new ServerSlackNotifier(process.env.SLACK_CHANNEL_LINK);

let loggers = {
  root: 5 /* TRACE by default */,
};
...
class Logger {
  ...
  static _log(clientserver, identifier, level, message, data, userid) {
    Meteor.defer(() => {
      ...
      if (level <= 1) {
        serverSlackNotifier.sendLog({ message: msg, date: now });
      }
...
Meteor.startup(() => {
  ...
  mongoLogger.find({ source: { $exists: 1 } }).observe({
    changed(newdoc) {
