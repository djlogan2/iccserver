...
describe.only("Polyglot class", function(done){
  const polyglot = new PolyGlot("/users/davidlogan/workspace/icc/stockfish/docker/book.bin");
  it("should find hash values correctly", function(){
    polyglot.initialize()
      .then(() => polyglot.getBookMoves("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"))
      .then((entries) => {
        done();
      });
