...
export class s3FileCollection {
  constructor(config) {
    ...
    if (config.updateVersionMetaData && typeof config.updateVersionMetaData === "function")
      this.updateVersionMetaData = config.updateVersionMetaData;
    ...
    Object.keys(fileRef.versions).forEach((version) => {
      ...
      s3.upload(
        ...
        Meteor.bindEnvironment((err, data) => {
          ...
          const update = !!this.updateVersionMetaData
            ? this.updateVersionMetaData(fileRef, version)
            : null;
          if (!!update) this.filesCollection.update({ _id: fileRef._id }, update);
