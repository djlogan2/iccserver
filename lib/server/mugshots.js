...
Mugshots.deleteMugshot = function (message_identifier, mugshot_id) {
  ...
  if (!mugshot) {
    ClientMessages.sendMessageToClient(user, message_identifier, "FILE_DOES_NOT_EXIST");
    return;
  }
  const roles = ["delete_any_mugshot"];
  if (mugshot.userId === user._id) roles.push("delete_mugshot");

  if (!Users.isAuthorized(user, roles)) {
    ClientMessages.sendMessageToClient(user, message_identifier, "NOT_AUTHORIZED");
    return;
  }

  // The delete above will handle this update.
  // Meteor.users.update({ _id: mugshot.userId }, { $unset: { mugshot: 1 } });
  Mugshots.remove({ _id: mugshot_id });
};

Picker.route("/mugshot/:mugshotid", (params, req, res) => {
  const mugshot = Mugshots.findOne({ _id: params.mugshotid });
  if (!mugshot?.meta?.validated) {
    ...
    // res.writeHead(404);
    // res.end("File " + params.mugshotid + " was not found");
    return;
  }
  const link = !!s3bucket
    ? "https://" + s3bucket + ".s3.amazonaws.com/" + mugshot._id + ".original"
    : mugshot.link();
