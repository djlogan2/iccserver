      ...
      $set: {
        pending: {
          white: {
            ...
            takeback: { number: data.wtakeback, mid: 0 }
          },
          black: {
            abort: data.babort ? "1" : "0",
            draw: data.bdraw ? "1" : "0",
            adjourn: data.badjourn ? "1" : "0",
            takeback: { number: data.btakeback, mid: 0 }
          }
        }
      },
      $push: {
...
LegacyUser.declineAdjourn = function(message_identifier, game_id) {
  ...
  if (Meteor.userId() in legacy_users) {
    ...
    LegacyUser.resetPending(game_id);
...
//Added this helper method for now to reset the game's pending requests on an
//accepted pending request
LegacyUser.resetPending = function(game_id) {
...
Meteor.publishComposite("loggedOnUsers", {
  ...
  children: [
    {
      find(user) {
        if (!Users.isAuthorized(user, "show_users") || !!user.status.legacy) return Meteor.users.find({_id: "0"});
        else {
          return Meteor.users.find(
            {
              $and: [
