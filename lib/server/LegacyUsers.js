import legacy from "@chessclub.com/legacy";
...
function move(data) {
  ...
  if (!game) throw new Meteor.Error("Unable to find a game for move");

  if (game.tomove === "white" && game.pending.black.takeback > 0) {
    game.pending.black.takeback.count = 0;
  }
  if (game.tomove === "black" && game.pending.white.takeback.number > 0) {
    game.pending.white.takeback.number = 0;
  }
...
function takeback(data) {
  ...
  const count = game.pending.white.takeback.number || game.pending.black.takeback.number;

  for (let x = 0; x < count; x++) {
    variation.cmi = variation.movelist[variation.cmi].prev;
    variation.movelist.pop();
    delete variation.movelist[variation.cmi].variations;
  }

  const chess = new Chess.Chess();
  for (let i = 1; i < variation.movelist.length; i++) {
    chess.move(variation.movelist[i].move);
  }
  ...
  Game.GameCollection.update(
    { legacy_game_number: data.gamenumber, status: "playing" },
    {
      $set: {
        fen: chess.fen(),
        "pending.white.takeback": { mid: "0", number: 0 },
        "pending.black.takeback": { mid: "0", number: 0 },
...
function offers_in_my_game(data) {
  ...
  if (data.wtakeback >= 1 || data.btakeback >= 1) {
...
Meteor.startup(() => {
  Users.events.on("userLogin", function (fields) {
    log.debug("User is in legacy_login role", Users.isAuthorized(fields.userId, "legacy_login"));
  ...
  Users.events.on("userLogout", (fields) => {
    LegacyUser.logout(fields.userId);
  });
});
...
LegacyUser.acceptTakeback = function (message_identifier, game_id) {
  ...
  } else Game.acceptLocalTakeback(message_identifier, game_id);
};
...
LegacyUser.addGameMove = function (message_identifier, game_id, move, variation) {
  log.debug("addGameMove", { game_id: game_id, move: move, variation: variation });
  if (Meteor.userId() in legacy_users) {
    legacy_users[Meteor.userId()].move(message_identifier, move);
  } else Game.saveLocalMove(message_identifier, game_id, move, variation);
};
LegacyUser.removePremove = function (message_identifier, game_id) {
  log.debug("addGameMove", { game_id });
  if (Meteor.userId() in legacy_users) {
    // legacy_users[Meteor.userId()].move(message_identifier, move); TODO: this stuff
  } else Game.removeLocalPremove(message_identifier, game_id);
};
Meteor.methods({
  ...
  // eslint-disable-next-line meteor/audit-argument-checks
  addGameMove: (message_identifier, game_id, move, variation) =>
    LegacyUser.addGameMove(message_identifier, game_id, move, variation),
  // eslint-disable-next-line meteor/audit-argument-checks
  removeLocalPremove: (message_identifier, game_id) =>
    LegacyUser.removePremove(message_identifier, game_id),
});
