...
Timestamp.averageLag = function(userid) {
  check(userid, String);
  if (!users[userid]) throw new Meteor.Error("Unable to retrieve average lag for user " + userid, "User not found");

  const sessionid = users[userid];
  if (!sessions[sessionid]) throw new Meteor.Error("Unable to retrieve average lag for user " + userid, "Session not found");
...
Meteor.startup(function() {
  ...
  Users.addLoginHook(function(user, connection) {
    ...
    if (users[user._id]) {
      ...
      log.debug("Changing session id for user " + user._id + " from " + old_session + " to " + new_session);
    ...
    const ts_client = new TimestampClient("lib/server", (key, msg) => {
      protocol.send(key, msg, users[user._id]);
    });
