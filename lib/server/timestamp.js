...
Timestamp.averageLag = function(userid) {
  ...
  return sessions[sessionid].averageLag();
};
...
Timestamp.pingTime = function(userid) {
  ...
  return sessions[sessionid].lastPing();
};
...
Meteor.startup(function() {
  ...
  protocol.on(
    ...
    Meteor.bindEnvironment((message, sessionId) => {
      if (!sessions[sessionId])
        throw new Meteor.Error("Unable to find session id");
  ...
  Users.addLogoutHook(function(userId) {
    if (!users[userId])
      throw new Meteor.Error("We cannot find a user for timestamp logout hook");
    const sessionid = users[userId];
    if (!sessionid)
      throw new Meteor.Error(
        "We cannot find a session id for timestamp logout hook"
      );
