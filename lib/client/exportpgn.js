...
function allVariations(movelist, cmi, move_number, white_to_move) {
  ...
  variations.forEach((v) => {
    string += "(" + thisMove(movelist[v], move_number, true, white_to_move, v);
...
export function exportGameObjectToPGN(game) {
  ...
  if (!game.clocks) {
    ...
    if (!game.clocks.white.delaytype) game.clocks.white.delaytype = "none";
    if (!game.clocks.black.delaytype) game.clocks.black.delaytype = "none";

    switch (game.clocks.white.delaytype) {
      case "none":
        pgn += '"[TimeControl ' + game.clocks.white.initial + '"]\n';
        break;
      case "us":
      case "bronstein":
      case "inc":
