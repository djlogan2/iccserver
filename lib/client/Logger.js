...
let loggedOnUserId = null;

Accounts.onLogin(function(user){
    loggedOnUserId = user.user._id;
});
...
class Logger {
    ...
    static _check(identifier, level, message, data, userid) {
        ...
        if(lvl >= level)
            Logger._log(identifier, lvl, message, data, userid);
    }

    // noinspection JSUnusedGlobalSymbols
    error(message, data, userid) {Logger._check(this.identifier, 1, message, data, userid);}
    // noinspection JSUnusedGlobalSymbols
    warn(message, data, userid) {Logger._check(this.identifier, 2, message, data, userid);}
    // noinspection JSUnusedGlobalSymbols
    info(message, data, userid) {Logger._check(this.identifier, 3, message, data, userid);}
    // noinspection JSUnusedGlobalSymbols
    debug(message, data, userid) {Logger._check(this.identifier, 4, message, data, userid);}
    // noinspection JSUnusedGlobalSymbols
    trace(message, data, userid) {Logger._check(this.identifier, 5, message, data, userid);}
}
