...
import ClientSlackNotifier from "./ClientSlackNotifier";

const clientSlackNotifier = new ClientSlackNotifier(process.env.SLACK_CHANNEL_LINK);
...
class Logger {
  ...
  static _log(identifier, level, message, data, userid) {
    ...
    if (level <= 1) {
      clientSlackNotifier.sendLog({ message, identifier, userId: userid });
    }
  }
