...
export const TimestampServer = function(log, debug, sender_function, lagging_function, count) {
  ...
  function pongArrived(response) {
    ...
    if (delay < 0 || clock_offset < -250 || clock_offset > 250) {
      log.error(
        "Error calcualting ping delay, delay=" +
          delay +
          ", arrival=" +
          arrival +
          ", originate=" +
          ...
          clock_offset +
          ", new clock_offset=" +
          current_clock_offset
      );
      delay = 0;
    }

    //
    // For safety, limit clock offsets to 1/10 of a second, and force timestamp to adjust
