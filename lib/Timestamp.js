...
export const TimestampServer = function(debug, sender_function, lagging_function, count) {
  ...
  function pongArrived(response) {
    ...
    if (!pendingrequests[response.id]) throw new Meteor.Error("Unable to find request for response");
  ...
  interval_id = Meteor.setInterval(() => {
    if (Object.entries(pendingrequests).length !== 0 && pendingrequests.constructor === Object && typeof _lagfunc === "function") Meteor.bindEnvironment(_lagfunc());
    sendPing();
    cleanupOldPings();
  }, 1000);
