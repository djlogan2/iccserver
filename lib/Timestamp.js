...
export const TimestampServer = function(log, debug, sender_function, lagging_function, count) {
  ...
  function pongArrived(response) {
    ...
    if (debugs.length === 60) debugs.shift();
    debugs.push(response);
    if (delay < 0) {
      log.error("Error calcualting ping delay, delay=" + delay);
      debugs.forEach(dbg => log.error("  response=" + JSON.stringify(dbg)));
      delay = 0;
    }
