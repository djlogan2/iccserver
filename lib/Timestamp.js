...
export const TimestampServer = function(
  sender_function,
  lagging_function,
  count
) {
...
/*
    const current = Object.keys(pendingrequests)
      .filter(key => pendingrequests[key].originate >= old)
      .map(key => pendingrequests[key]);
    //const current = pendingrequests.filter(req => req.originate >= old);
    //const oldcount = pendingrequests.length - current.length;
