...
export default class CircleAndArrow extends Component {
    render() {
        ...
        return (
            <div>
                <svg id="mouseArrorwSVG" style={{ zIndex: 99999, width: this.props.boardsize + "px", height: "100%", position: "fixed", top: 0, pointerEvents: "none" }}>
                </svg>
                <div id="circle-and-line-template" style={{ display: "none" }}>
                    <svg>
                        <defs>
                            <marker id="markerItem" orient="auto" markerWidth="4" markerHeight="8" refX="2.05" refY="2.01">
                                <path d="M0,0 V4 L3,2 Z" fill="#15781B"></path>
                            </marker>
                        </defs>
                        <line id="lineItem" stroke="#15781B" strokeWidth="9.0625" strokeLinecap="round"
                            markerEnd="url(#markerItem)" opacity="1" x1="0" y1="0" x2="0" y2="0"></line>
                        <circle id="circleItem" stroke="#15781B" strokeWidth="3.625" fill="none" opacity="1" cx="0" cy="0" r="0">
                        </circle>
                    </svg>
                </div>
            </div>
        ...
        $("#" + currentObject.props.chardBoardName).mousemove(function (event) {
            if (handleMouseMove == true) {
                ...
                if ($(target).attr(currentObject.props.squareId) != currentElementId) {
                    ...
                    $("#lineItem-" + currentClick).attr("x2", $(target).position().left + ($(target).width() / 2) - errorXAdjustment);
                    $("#lineItem-" + currentClick).attr("y2", $(target).position().top + ($(target).height() / 2));
